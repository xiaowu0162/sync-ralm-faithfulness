...
def main():
    ...
    pred_file_no_rag = f'{args.root_dir}/results/{args.task}/no-rag/{model_short}.json'
    # no_rag_prompts = [json.loads(line)['input'] for line in open(pred_file_no_rag).readlines()]
    no_rag_prompts = ['[INST] ' + json.loads(line)['input'] + ' [/INST]' for line in open(pred_file_no_rag).readlines()]

    # pred_file_rag = f'{args.root_dir}/results/{args.task}/always-rag/{model_short}.json'
    pred_file_rag = f'{args.root_dir}/data/task_model_split/{args.task}/output_only/{model_short}.jsonl'
    pred_data = [json.loads(line) for line in open(pred_file_rag).readlines()]
    # rag_prompts = [json.loads(line)['input'] for line in open(pred_file_rag).readlines()]
